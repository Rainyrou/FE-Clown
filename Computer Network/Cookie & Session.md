HTTP 是无状态的，一旦完成数据交换，就会关闭本次连接，再次交换数据就需要重新连接，即服务端无法从连接上跟踪会话，但在某些场景下我们需要维护状态。确定用户身份需要跟踪用户的整个会话，常用的方式是使用 Cookie 与 Session，简单来说 Cookie 通过在客户端记录信息确定用户身份，Session 是在服务端中执行类似的操作
