...
- 词法分析：词法分析器依照语法规则扫描源代码文本结构，移除源代码的非语法细节如空格、注释和分号，拆分为 Token
- 语法分析：语法分析器依照语法规则验证 Token 的语法合法性并据此建立 Token 间的嵌套关系，将其转化为 AST
- 遍历机制：遍历器通过 DFS 遍历 AST，当遇到某类型节点时调用访问者对象对应类型的处理函数，传入节点对象及上下文，对其进行增删修改操作
