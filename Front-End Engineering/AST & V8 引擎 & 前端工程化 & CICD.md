...
- 增删修改：遍历器 DFS AST，当访问某类型节点时调用访问者对象对应的处理函数，增删修改 AST 节点

V8 执行 JavaScript 流程分为解析 → 编译 → 执行 → 优化/去优化阶段：

1. 解析：

- 词法分析：词法分析器根据词法规则扫描源代码文本结构，移除源代码的非语法细节如空格、注释和分号，拆分为 Token
- 语法分析：语法分析器根据语法规则验证 Token 的语法合法性并据此建立 Token 间的嵌套关系，将其转化为 AST

2. 编译：Ignition 遍历 AST，将 AST 转换为字节码，再将字节码转换为基线机器码，热点探测器标记热点代码，TurboFan 获取热点代码的 AST、字节码和 Ignition 记录的执行信息，通过优化编译将基线机器码转换为优化机器码
3. 执行：CPU 执行优化机器码，读写调用栈和堆，当访问函数调用时，创建新的栈帧并压入调用栈，函数执行完后弹出，恢复上一层函数的执行上下文
4. 优化/去优化：

- 逃逸分析：TurboFan 判断变量是否逃逸（是否全局引用、传递至其他函数或存储至堆中），未逃逸的变量压入调用栈
- 函数内联：将频繁调用的函数内嵌至调用方代码，减少函数调用的压栈和出栈开销
- 内联缓存：记录函数参数类型和对象属性访问路径，后续直接复用缓存
- 去优化：若变量类型与 TurboFan 的推测不同，则移除优化机器码，回退至 Ignition 生成的基线机器码执行并更新执行信息
