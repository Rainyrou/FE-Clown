MCP：面向 LLM 的上下文标准化交互协议，将所有类型的上下文抽象为标准化最小单元 Context Unit，该单元为协议载体，包含全局唯一 context_id、模态类型、会话关联、优先级等元数据，以及模态适配的标准化编码内容（文本为 UTF-8+Token 索引，多模态为语义嵌入向量 + 原始数据哈希），MCP 通过增量传输（仅传递新增 Context Unit，而非全量上下文）机制实现上下文交互，通过 Protobuf/FlatBuffers 二进制序列化减少编解码性能开销，基于分级缓存（L1 本地内存/L2 分布式缓存/L3 持久化存储）实现上下文高效读取，当上下文超过模型窗口阈值则触发优先级截断和语义压缩

Agent：将 LLM 从概率统计文本生成器升级为有环境感知与逻辑决策能力的控制器，Agent 架构遵循"感知 - 规划 - 行动 - 记忆" 机制，LLM 将用户目标拆分为可执行的子任务序列，通过上下文窗口维持短期记忆并通过向量数据库结合 Embedding 技术实现长期记忆的持久化存储与语义检索，Agent 基于 Reasoning + Acting 范式或 Function Calling 机制，将自然语言描述映射为结构化的 API 调用指令，调度外部工具与环境交互并将执行结果作为新上下文回传给模型以修正下一步策略，形成闭环，直至目标状态或触发终止条件
...
- 上下文分层存储，根据优先级从高到低依次为系统提示词 -> 用户当前输入 -> 关联历史上下文 -> 资源索引 Token，通过滑动窗口 + 优先级排序机制，实时计算上下文 Token 总数，超过模型窗口阈值则触发优先级截断和语义压缩
- 资源分片上传，通过规则引擎过滤非法资源，验证资源格式与模型兼容性，资源存储于分布式对象存储，生成唯一资源 URL 并与会话 ID 绑定
- 客户端将会话 ID + 结构化上下文封装为 API 请求并发送给服务端，服务端接收到请求后，通过会话 ID 检索上下文容器，验证资源索引的有效性及 Token 总数是否超过模型窗口阈值，同时将结构化 Token 序列输入模型推理引擎触发前向计算生成响应 Token，将本次用户需求与模型响应追加至上下文容器，更新 Token 总数与时间戳
