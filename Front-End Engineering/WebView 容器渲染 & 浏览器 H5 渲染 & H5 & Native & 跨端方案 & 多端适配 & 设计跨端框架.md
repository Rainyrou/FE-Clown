WebView 容器渲染 vs 浏览器 H5 渲染：

- 明确需求：WebView 容器与浏览器渲染的渲染链路差异，主要体现在以下维度：
- 核心逻辑：
  1. 内核层面：浏览器采用自研内核，功能全面且自主更新，WebView 采用操作系统内置内核如 iOS WebKit，Android Chromium，功能和版本受限，不提供地址栏、导航栏和书签管理器
  2. 渲染层面：浏览器自主渲染 URL 解析 → 网络请求 → 渲染流水线，WebView 渲染的优先级低于原生，且由容器触发初始化，受容器进程和生命周期控制
  3. 资源层面：浏览器有完整的资源缓存（如 HTTP 缓存、Disk/Memory Cache）和预加载机制；WebView 缓存受容器控制
- 落地细节：我在做营销 H5 页面的 WebView 优化时，同一 H5 页面在 Chrome 浏览器首屏渲染 800ms，而在容器内 WebView 耗时 1.5s，排查发现是容器拦截资源进行鉴权 + WebView 内核版本过低不支持新 CSS 特性，通过升级内核 + 优化拦截逻辑后，首屏耗时降至 1s 内
- 避坑点：WebView 容易因为容器内存限制和进程回收导致渲染中断，且不同系统版本的 WebView 对 H5 新特性兼容性迥异，需进行版本检测和降级处理

H5/Native/跨端方案：

- 结论（10s）：我认为核心在于解决不可能三角的平衡问题，Native 追求用户体验，H5 追求敏捷开发，跨端追求投入产出比，取舍的本质为业务场景的优先级匹配
- 落地细节（240s）：基于不同业务实践，我给出三类方案的优劣与取舍逻辑：
  ...
  2. 跨端：核心优势是「开发效率与用户体验的平衡」—— 一次开发多端复用，开发成本比 Native 低 50%，性能却接近于 Native；核心劣势是「生态依赖 + 定制成本高」——跨端框架生态远逊于 H5/Native，需要自研封装，复杂原生能力（如 AR/游戏）适配难度大，且多端一致性需要额外打磨
  3. Native：核心优势是「性能拉满 + 能力全量 + 体验原生」—— 跟手动画稳定 60fps，支付/风控等核心业务场景安全性最高，系统 API 调用无限制；核心劣势是「开发成本翻倍 + 动态化缺失」—— 双端各自独立开发维护，发版周期依赖应用商店审核，紧急 Bug 修复无法快速上线

* 取舍逻辑（45s）：基于上述思考和业界调研，我会在实战中设计 APP 融合型架构，底层由 Native 提供高性能容器和系统桥接，以及承接支付/风控等核心业务，中层通过跨端方案承载 80% 的中高频业务，上层通过 H5 承接营销活动和官网

多端适配：

- 结论（10s）：我认为多端适配的核心在于工程提效与分层降级
