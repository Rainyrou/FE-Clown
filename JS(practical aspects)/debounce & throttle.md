防抖 `debounce` 与节流 `throttle` 都是控制事件处理函数执行频率的方法。在面对高频触发的事件（如 `onscroll`）时，如果其处理函数涉及资源密集型操作（如 DOM 操作或服务器请求），不加控制地频繁执行可能导致性能问题。注意，防抖和节流并不能减少事件触发次数，而是通过有策略地限制处理函数的执行次数，从而提高性能
