...
- TCP 可靠传输
- WebSocket 协议层增强：建立连接只需客户端和服务端一次 HTTP 握手，而后通过 WebSocket 连接通信，客户端发送  `Upgrade: websocket` + `Sec-WebSocket-Key`  请求头部，服务器返回  `101 Switching Protocols` + 加密后的  `Sec-WebSocket-Accept`  响应头部；发送方发送 g 关闭帧和关闭码，接收方返回 ACK 报文后关闭连接
