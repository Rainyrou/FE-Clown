...
- Number：JavaScript 通过 64 位双精度浮点格式（-2^53 至 2^53 - 1），`Infinity`，`-Infinity` 和 `NaN` 表示所有数字
- String：字符串不可变
- Symbol：唯一值如对象唯一属性键
- BigInt：无穷大无穷小数
...
* 递归调用 `Object.freeze` 阻止增删修改嵌套对象属性
* 通过 `Immutable.js/Immer.js` 和深拷贝将对象转换为不可变数据结构

浮点精度问题：JavaScript 通过 64 位双精度浮点格式（-(2^53 - 1) 至 2^53），`Infinity/-Infinity/NaN` 表示所有数字，1 位表示符号，11 位表示指数，52 位表示小数，超出内存限制的部分截断
...
JavaScript 隐式转换的核心为「求原始值」，依赖于以下底层操作：
...
1. `ToNumber`  或  `ToString`  或  `ToBoolean`：根据上下文将原始值转换为目标类型
