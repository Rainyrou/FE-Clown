...
- `getCount` 计算在以  `node`  为前缀的的所有数字中 ≤n 的数字总数，10 叉树节点按层分布，各层节点范围为  `[left, right)`，第 0 层为 `node` 本身
- 初始化 `--k`，剩余位置 k -> 0 - based 索引，`k = 0` 即当前节点
- `count ≤ k`，子树索引范围为「0 ~ count - 1」，若 `k ≥ count` → 目标索引超出子树索引范围
- 字典序 + 十叉树的时间复杂度 O (log²n)，堆排序的时间复杂度为 O(k log 10)
