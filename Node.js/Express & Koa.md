Express：最成熟的 Node.js 框架之一，拥有丰富的中间件生态，其中间件以线性方式组织，接收 `res`、`req` 和 `next` 参数，通过 `next` 和回调函数显式调用下一中间件，或不调用 `next` 中断执行，若中间件抛出错误，调用 `next(error)` 跳过后续所有非错误处理的中间件，直接将控制权传递给 `(err, req, res, next)` 四参数的错误处理中间件（位于中间件栈的最底部）
